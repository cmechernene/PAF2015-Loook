...
CColorBasics::CColorBasics() :
    m_pD2DFactory(NULL),
    m_pDrawColor(NULL),
    m_hNextColorFrameEvent(INVALID_HANDLE_VALUE), //detecte evenement camera video
    m_pColorStreamHandle(INVALID_HANDLE_VALUE),
    m_bSaveScreenshot(false),
    m_pNuiSensor(NULL) //capteur a utiliser
{
}
...
CColorBasics::~CColorBasics()
{
    if (m_pNuiSensor)
    {
        m_pNuiSensor->NuiShutdown(); //eteint camera kinect
    }
...
HRESULT CColorBasics::CreateFirstConnected()
{
    ...
    int iSensorCount = 0;
    hr = NuiGetSensorCount(&iSensorCount); //compte le nombre de capteurs de la kinect, résultat dans iSensorCount
    if (FAILED(hr))
    {
        return hr;
    }
    ...
    if (NULL != m_pNuiSensor)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = m_pNuiSensor->NuiImageStreamOpen(
                NUI_IMAGE_TYPE_COLOR, //donnée RGB [https://msdn.microsoft.com/en-us/library/nuiimagecamera.nui_image_type.aspx ]
